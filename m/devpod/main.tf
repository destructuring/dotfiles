    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          command = ["/usr/bin/tini", "--"]
          args    = ["bash", "-c", "(echo \"https://${each.key}-0.${each.value.domain} {\"; echo 'reverse_proxy http://localhost:8888'; echo '}'; echo \"https://${each.key}-0.${each.value.domain}:8881 {\"; echo 'reverse_proxy http://localhost:4646'; echo '}';) > Caddyfile; exec sudo `~ubuntu/bin/e asdf which caddy` run"]
        ...
        container {
          ...
          command = ["/usr/bin/tini", "--"]
          args    = ["bash", "-c", "while true; do ts_ip=`tailscale ip -4 || true`; if test -n \"$ts_ip\"; then break; fi; sleep 1; done; exec nomad agent -config=etc/nomad.conf -data-dir=/work/nomad -dc=grove -region=spiral -node=grove-0 -bootstrap-expect 1 -bind \"$ts_ip\""]
