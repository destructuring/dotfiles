    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          command = ["/usr/bin/tini", "--"]
          args    = ["bash", "-c", "while true; do (echo \"https://${each.key}-0.${each.value.domain} {\"; echo \"handle_path /dev/* {\n reverse_proxy http://localhost:10350\n }\"; echo 'reverse_proxy http://localhost:8000'; echo '}'; ) > Caddyfile; exec sudo `~ubuntu/bin/e asdf which caddy` run"]
