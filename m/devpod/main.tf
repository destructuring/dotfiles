    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          command = ["/usr/bin/tini", "--"]
          args    = ["bash", "-c", "while true; do if test -S /var/run/tailscale/tailscaled.sock; then break; fi; sleep 1; done; sudo tailscale up --ssh --accept-dns=false --hostname=${each.key}-0; make symlinks perms; exec ~/bin/e code-server --bind-addr 0.0.0.0:8888 --disable-telemetry"]
